...
Analisar experimentalmente o impacto das estruturas de particionamento espacial BVH e KD-Tree no tempo de renderização de um ray tracer, por meio do desenvolvimento e da comparação de implementações.

\subsection{Objetivos secundários}

\begin{itemize}
    \item Desenvolver um ray tracer básico em C++, capaz de renderizar cenas tridimensionais contendo diferentes quantidades de objetos geométricos.
    \item Implementar estruturas de aceleração baseadas em BVH (Bounding Volume Hierarchy) e KD-Tree, integrando-as ao processo de interseção de raios com objetos da cena.
    \item Comparar o desempenho das duas estruturas de particionamento espacial, analisando a redução no número de testes de interseção entre raios e primitivas geométricas.
    \item Avaliar o impacto de cada estrutura no tempo total de renderização, considerando diferentes níveis de complexidade das cenas.
    \item Analisar o custo de construção das estruturas BVH e KD-Tree e seu impacto no tempo total do processo de renderização.
    \item Investigar como fatores como número de objetos, profundidade da árvore e organização espacial da cena influenciam o desempenho das estruturas.
\end{itemize}
